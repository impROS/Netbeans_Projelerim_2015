...
//		}		
		...
		if(game.isGameOver() || game.isNewGame() || game.isPaused()) {
			...
			if(game.isNewGame()) {
				largeMessage = "Yılan Oyunu!";
				smallMessage = "Başlamak İçin Enter a Basınız";
			} else if(game.isGameOver()) {
				largeMessage = "Oyun Bitti! Toplam Skor = " + game.getScore();
				smallMessage = "Tekrar Başlamak İçin Enter a Basınız";
			} else if(game.isPaused()) {
				largeMessage = "Durduruldu";
				smallMessage = "Devam Etmek İçin Enter a basınız";
			}
	 ...
	 */
        public static void renkAta(TileType type, Graphics g) {
       g.setColor(Color.WHITE);
        }
